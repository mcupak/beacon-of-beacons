...
# Add the datasource
data-source add --name=bob --driver-name=mysql --jndi-name=java:jboss/datasources/bob --connection-url=jdbc:mysql://${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DATABASE}?useUnicode=true&amp;characterEncoding=UTF-8 --user-name=${MYSQL_USERNAME} --password=${MYSQL_PASSWORD} --use-ccm=false --jta=true --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
